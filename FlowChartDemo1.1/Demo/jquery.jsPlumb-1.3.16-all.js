    ...
    var log = null,
		...
		_draw = function(element, ui, timestamp) {
            ...
            if (!jsPlumbAdapter.headless && !_suspendDrawing) {
                ...
                var thisemement  = _getElementObject(element);
                if(thisemement.attr('class').indexOf("lzemay_wModeS")>0&&thisemement.attr('class').indexOf("draggable-dragging")>0)
                {
               
                var pjleft = Math.floor(thisemement.position().left);
                var pjtop = Math.floor(thisemement.position().top);
                $(".lzemay_wModeS").each(function (index, element) { 
                     if (id != this.id) {
                        if (Math.floor($(this).position().left)+10 > pjleft&&Math.floor($(this).position().left)-10 < pjleft) {

                            var positionleftvalue =  $(this).position().left;
                            var scrollleftvalue = $("#demo_emdrap").scrollLeft();
                            var demrapleft = $("#demo_emdrap").offset().left;
                            
                            $("#postionconfirex").show();
                            $("#postionconfirex").css("left", positionleftvalue+demrapleft);
                            $("#"+id).css("left", positionleftvalue+scrollleftvalue);
                            
                            return false;
                        }else {
                            $("#postionconfirex").hide();
                        }
                    } }); 
                      ...
                      $(".lzemay_wModeS").each(function (index, element) { 
                    if (id != this.id) {
                        if (Math.floor($(this).position().top)+10 >pjtop&&Math.floor($(this).position().top)-10< pjtop) {

                            var postiontopvalue  =$(this).position().top;
                            var scrolltopvalue = $("#demo_emdrap").scrollTop();
                            var demraptop = $("#demo_emdrap").offset().top;
                            $("#postionconfirey").show();
                            $("#postionconfirey").css("top", postiontopvalue+demraptop);
                            $("#"+id).css("top", postiontopvalue+scrolltopvalue+demraptop);
                              console.log(scrolltopvalue);
                            return false;
                        }else{
                          $("#postionconfirey").hide();
                        }
                    } });
               }
				...
				if (draggable) {
					if (jpcl.isDragSupported(element) && !jpcl.isAlreadyDraggable(element)) {
						...
						options[stopEvent] = _wrap(options[stopEvent], function() {
                            ...
                            Config.DraggableDrapDiv(null,null,_getElementObject(element)[0],null);
                           // window.setTimeout(Config.DraggableDrapDiv(null,null,_getElementObject(element)[0],null),100);
                            $("#postionconfirex").hide();
                            $("#postionconfirey").hide();
						});
